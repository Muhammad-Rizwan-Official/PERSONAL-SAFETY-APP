...
# Importing necessary classes from other files
from trackers import LocationTracker, VoiceInteraction
...
class PersonalSafetyApp(QMainWindow):
    def __init__(self):
        ...
        self.voice_interaction.finished.connect(self.handle_recognition_result)
    ...
    def start_voice_interaction(self):
        ...
        print("Voice interaction thread started.")



    def handle_recognition_result(self, text):
        """
        This method will handle the result emitted by the VoiceInteraction's finished signal.
        The `text` parameter will contain the recognized command or response.
        """
        print(f"Recognized command: {text}")
        # Depending on the recognized command, you can trigger different actions here
        if "sos" in text.lower():
            self.activate_sos()
        elif "cancel" in text.lower() and "sos" in text.lower():
            self.cancel_sos()
        elif "check in" in text.lower():
            self.safe_check_in()
        elif "location" in text.lower():
            self.update_location()
        elif "emergency" in text.lower() and "call" in text.lower():
            self.call_emergency_services()
        elif "nearby" in text.lower() and "safe" in text.lower():
    ...
    def initUI(self):
        ...
        self.voice_command_button.clicked.connect(self.start_voice_interaction)
